...
export async function checkFranklinCountyStatus(pickupAddress, destinationAddress) {
  try {
    // PRIORITY SYSTEM: Pattern matching overrides Google API
    ...
    // These 21 patterns are the FINAL AUTHORITY - Google API is ignored
...
export function calculateTripPrice({
  ...
  let breakdown = {
    ...
    roundTripPrice: 0,
    distancePrice: 0,
    tripDistancePrice: 0,         // Alias for BookingForm display
    countyPrice: 0,
    countySurcharge: 0,           // Alias for BookingForm display
    deadMileagePrice: 0,
    weekendAfterHoursSurcharge: 0,
    weekendSurcharge: 0,          // Alias for BookingForm display
    afterHoursSurcharge: 0,       // Alias for BookingForm display
    emergencyFee: 0,
    emergencySurcharge: 0,        // Alias for BookingForm display
    holidaySurcharge: 0,
    wheelchairPrice: 0,
    veteranDiscount: 0,
    total: 0,
  ...
  if (distance > 0) {
    ...
    breakdown.tripDistancePrice = breakdown.distancePrice; // Alias for BookingForm
  }
  ...
  if (countyInfo && countyInfo.countiesOut >= 2) {
    ...
    breakdown.countySurcharge = breakdown.countyPrice; // Alias for BookingForm
  }
  ...
  if (pickupDateTime) {
    ...
    if (isAfterHoursTime || isWeekendTime) {
      ...
      // For BookingForm: show which condition triggered it
  ...
  if (isEmergency) {
    ...
    breakdown.emergencySurcharge = breakdown.emergencyFee; // Alias for BookingForm
  }
