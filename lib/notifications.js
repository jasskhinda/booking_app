...
async function notifyDispatchersOfNewTrip(trip, user) {
  try {
    ...
    // Also send push notifications to dispatcher mobile app
    try {
      const pushResponse = await fetch(`${process.env.NEXT_PUBLIC_DISPATCHER_APP_URL}/api/notifications/send-dispatcher-push`, {
        method: 'POST',
        headers: {
