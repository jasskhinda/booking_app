...
export async function notifyDispatchersOfNewTrip(trip, user) {
  try {
    ...
    if (!dispatcherEmails.length) {
      ...
      // In production, log the issue but don't fail the trip creation
      console.error('Production: No dispatchers configured - trip created but no notifications sent');
      return { 
        success: true, 
        info: 'Trip created successfully. Dispatcher notifications unavailable.' 
      };
    }
    ...
    const html = `
      ...
      <p>Please <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://compassionate-transportation.vercel.app'}/dashboard" style="color: #2563eb; text-decoration: underline;">log in to the dispatcher portal</a> to approve or decline this trip request.</p>
    `;
