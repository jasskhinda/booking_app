...
export default function LoginForm() {
  ...
  useEffect(() => {
    ...
    } else if (errorParam === 'Authentication failed') {
  ...
  const handleSignInWithGoogle = async () => {
    ...
    try {
      // Get the current origin dynamically to handle both local and production URLs
    ...
    <form onSubmit={handleSubmit} className="mt-8 space-y-6">
      ...
      <div className="space-y-3">
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full flex justify-center items-center gap-3 py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#5fbfc0] hover:bg-[#4aa5a6] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#5fbfc0] disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isLoading ? (
            <div className="h-5 w-5 animate-spin rounded-full border-2 border-gray-300 border-t-[#5fbfc0]"></div>
          ) : (
