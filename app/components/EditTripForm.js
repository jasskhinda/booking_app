...
export default function EditTripForm({ trip, onSave, onCancel }) {
  ...
  useEffect(() => {
    ...
    const getCurrentUser = async () => {
      ...
      if (user) {
        const { data: profile } = await supabase
          ...
          .eq('id', user.id)
          .single();
        setCurrentUser({ ...user, role: profile?.role || 'client' });
      }
    };
    getCurrentUser();
    ...
    if (trip) {
      setSelectedClient({
        client_type: 'individual', // BookingCCT is for individual clients
        id: trip.user_id
      });
    }
  }, [trip]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Check if trip is still pending (only pending trips can be edited by clients)
      ...
      // Clients can only edit pending trips
      if (currentTrip.status !== 'pending') {
        throw new Error('This trip can no longer be edited. Only pending trips can be modified by clients.');
      }
      ...
      const updateData = {
        ...
        edited_by_role: currentUser?.role || 'client',
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  min={today}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#5fbfc0]"
                  required
                  disabled={loading}
                />
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#5fbfc0]"
                  disabled={loading}
                />
              </div>
            ...
            <div>
              <label className="flex items-center space-x-3">
                <input
                  ...
                  className="w-4 h-4 text-[#5fbfc0] border-gray-300 rounded focus:ring-[#5fbfc0]"
                  disabled={loading}
                />
